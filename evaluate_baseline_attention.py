#!/usr/bin/env python3
"""
Evaluation Script for Baseline VQA Model Variants (ResNet34 + BERT)
Same comprehensive evaluation as lightweight models - UNIFIED OUTPUT
"""
...
def _checkpoint_path() -> Path:
    ...
    if best_path.exists():
        ...
        # Return best_path as default (will show error if neither exists)
        return best_path


class BaselineModelEvaluator:
    """Evaluator for baseline VQA models - outputs match evaluate_saved_model."""
    ...
    def __init__(self,
        ...
        self.architecture = f'baseline_{self.variant_name}'
        ...
        print(f"✓ Loaded baseline model: {self.variant_name}")
    ...
    def _extract_variant_name(self):
        ...
        for part in parts:
            if part.startswith('baseline_'):
                return part.replace('baseline_', '')
        return 'attention'
    ...
    def _load_model(self):
        ...
        model = BaselineVQAModel(
            num_classes=self.config.get('num_classes', len(self.answer_vocab)),
            visual_feature_dim=self.config.get('visual_feature_dim', 512),
        ...
        print("  Architecture: ResNet34 + BERT + attention fusion")
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate Baseline Attention VQA Model')
    
    parser.add_argument('--checkpoint', type=str, default=None,
                       help='Path to a checkpoint (defaults to baseline_attention/checkpoint_best.pth)')
