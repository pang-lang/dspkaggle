...
class MatchedBaselineTrainer:
    ...
    def calculate_class_weights(self, train_loader):
        """Calculate class weights with gentler rebalancing."""
        ...
        # GENTLER weighting scheme
        weights = torch.zeros(self.num_answers)
        for label in range(self.num_answers):
            count = label_counts.get(label, 0)
            if count > 0:
                # Less aggressive smoothing
                weights[label] = np.log1p(total / count)  # Gentler than sqrt
            else:
                weights[label] = 0.5
        
        weights = weights / weights.mean()
        weights = torch.clamp(weights, min=0.5, max=3.0)  # Narrower range
    ...
    def initialize_model(self, train_loader):
        ...
        self.scheduler = OneCycleLR(
            self.optimizer,
            max_lr=self.learning_rate * 5,  # Peak at 5x base LR
