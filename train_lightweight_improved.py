#!/usr/bin/env python3
"""
Improved Lightweight Training Script
Fixes: vocabulary size, regularization, learning rate schedule
"""
...
class ImprovedFocalLoss(nn.Module):
    """Focal Loss with gentler class rebalancing."""
...
class ImprovedLightweightTrainer:
    ...
    def __init__(self,
                 ...
                 num_epochs: int = 50,
                 device: str = None,
                 checkpoint_dir: str = 'checkpoints/lightweight_improved',
    ...
    def calculate_class_weights(self, train_loader: DataLoader) -> torch.Tensor:
        ...
        for label in range(self.num_answers):
            ...
            if count > 0:
                # Gentler weighting
                weights[label] = np.log1p(total / count)
            else:
                weights[label] = 0.5
    ...
    def initialize_model(self, train_loader: DataLoader):
        ...
        self.logger.info("=" * 80)
        self.logger.info("IMPROVED LIGHTWEIGHT TRAINING")
        self.logger.info("=" * 80)
        self.logger.info(f"Device: {self.device}")
        self.logger.info(f"Fixes: Better vocab coverage, less regularization, LR warmup")
        self.logger.info("=" * 80)
    ...
    def save_checkpoint(self, epoch: int, val_loss: float, val_acc: float, is_best: bool = False):
        ...
        checkpoint = {
            ...
            'config': {
                ...
                'architecture': 'lightweight_improved',
                'fusion_hidden_dim': self.fusion_hidden_dim,
                'num_attention_heads': self.num_attention_heads,
                'dropout': self.dropout,
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader):
        ...
        self.logger.info("\n" + "=" * 80)
        self.logger.info("IMPROVED LIGHTWEIGHT TRAINING START")
        self.logger.info("=" * 80)
...
def main():
    params = {
        ...
        'learning_rate': 1e-4,
        'num_epochs': 50,
        'max_answer_vocab_size': 150,  # Better coverage
        'fusion_hidden_dim': 384,
        'num_attention_heads': 6,
        'dropout': 0.3,  # Less regularization
        'focal_gamma': 2.0,  # Less extreme
        'weight_decay': 0.005,  # Less regularization
        'early_stopping_patience': 15
    }
    
    print("=" * 80)
    print("🔧 IMPROVED LIGHTWEIGHT TRAINING")
    print("=" * 80)
    print("\n📋 Configuration:")
    ...
    trainer = ImprovedLightweightTrainer(
    ...
    print("  python evaluate_with_roc.py --checkpoint checkpoints/lightweight_improved/best_model.pt")
